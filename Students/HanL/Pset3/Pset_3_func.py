...
#def get_cvec2(bvec, w, r, params):
#    S,  nvec, A, alpha, delta, beta, sigma, L, SS_tol = params
#    L = sum(nvec)
#    K = sum(bvec)
#    w = (1 - alpha) * A * (K / L) **alpha
#    r = alpha * A * (L / K)**(1 - alpha) - delta
#    cvec = np.array(nvec) * w + (1 + r) * np.concatenate(([0], bvec)) - np.append(bvec, [0])
#    return cvec
...
#    print(c_cnstr[len(error) - len(bvec): -1], len(c_cnstr[len(error) - len(bvec): -1]))
#    error[c_cnstr[len(error) - len(bvec):-1]] = 9999
    return error
#    
...
def get_SS(params, bvec_guess, SS_graphs):
    ...
    S,  nvec, A, alpha, delta, beta, sigma, L, SS_tol = params
    x = opt.fsolve(EulErrFunc, bvec_guess, args=(params), xtol = SS_tol)
    
#    x = opt.minimize(EulErrFunc, bvec_guess, args=(params), method ='Nelder-Mead', tol = 1e-12, options={'disp': False,   'maxfev': 5000000, 'maxiter' : 5000000})
#    print(x)
    b_ss = x
...
def non_ss (params, bvec_guess, maxper, path):
    ...
    for t in range(len(S) - 1):
        bvec_1.append((1.5 - 0.87) * (t) / 78 + 0.87)
    bvec_1 = np.array(bvec_1)
#    bvec_1 = np.array([0.8 * b_ss[0], 1.1 * b_ss[1]])
    ...
    while (norm(K_past, K_now) >= 0.004):
        ...
        if iter == 0:
            ...
            iter += 1
            print(format('iter: '), iter, format(' distance'), norm(K_past, K_now))
            ...
            print(format('iter: '), iter, format(' distance'), norm(K_past, K_now))
#    print(b_mat)
    return [K_now, w[0: maxper], r[0: maxper], b_mat]
#            
