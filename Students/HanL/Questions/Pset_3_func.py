...
def get_cvec(bvec, w, r, params):
    ...
    cvec = np.array(nvec) * w + (np.ones(len(S)) + r) * np.concatenate(([0], bvec)) - np.append(bvec, [0])
    return cvec
...
#    error = np.array([abs(i) for i in error]).sum()
#    print(error)
    return error
#    
...
def get_SS(params, bvec_guess, SS_graphs):
    ...
    x = opt.fsolve(EulErrFunc, bvec_guess, args=(params), xtol = math.e**(-14), maxfev = 100000000)
    
    
#    x = opt.minimize(EulErrFunc, bvec_guess, args=(params), method = 'L-BFGS-B')
    print(x)
    b_ss = x
