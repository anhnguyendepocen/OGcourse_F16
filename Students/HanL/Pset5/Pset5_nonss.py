...
# -*- coding: utf-8 -*-
"""
Created on Mon Nov  7 16:22:12 2016
...
def get_TPI(params, bvec1, graphs):
    ...
    (S, T, beta, sigma, chib, zeta_s, nvec, L, A, alpha, delta, b_ss, K_ss, C_ss,
        maxiter_TPI, mindist_TPI, xi, TPI_tol, EulDiff) = params

    K1, K1_cnstr = c10ssf.get_K(bvec1)
    ...
    w_params = (A, alpha)
    cbe_params = (S, T, beta, sigma, chib, zeta_s, nvec, bvec1, b_ss, TPI_tol,
                  EulDiff)

    while (iter_TPI < maxiter_TPI) and (dist_TPI >= mindist_TPI):
        iter_TPI += 1
        Kpath_init = xi * Kpath_new + (1 - xi) * Kpath_init
        rpath = c10ssf.get_r(r_params, Kpath_init, Lpath)
        wpath = c10ssf.get_w(w_params, Kpath_init, Lpath)
        cpath, bpath, EulErrPath = get_cbepath(cbe_params, rpath, wpath)
        Kpath_new = np.zeros(T + S - 2)
        Kpath_new[:T], Kpath_cnstr = c10ssf.get_K(bpath[:, :T])
    ...
    Kpath = Kpath_new
    Y_params = (A, alpha)
    Ypath = c10ssf.get_Y(Y_params, Kpath, Lpath)
    Cpath = np.zeros(T + S - 2)
    Cpath[:T - 1] = c10ssf.get_C(cpath[:, :T - 1])
    ...
    c10ssf.print_time(tpi_time, 'TPI')
...
def get_cbepath(params, rpath, wpath):
    ...
    --------------------------------------------------------------------
    '''
    S, T, beta, sigma, chib, zeta_s, nvec, bvec1, b_ss, TPI_tol, EulDiff = params
    ...
    for p in range(2, S):
        ...
        pl_params = (S, beta, sigma, chib, zeta_s, BQ, TPI_tol, EulDiff)
...
def paths_life(params, beg_age, beg_wealth, nvec, rpath, wpath,
    ...
    b_init     = (p,) vector, initial guess for remaining lifetime
                 savings
    ...
    --------------------------------------------------------------------
    '''
    S, beta, sigma, chib, zeta_s, BQ, TPI_tol, EulDiff = params
    p = int(S - beg_age + 1)
    ...
    b_guess =  1.01 * b_init
    eullf_objs = (beta, sigma, chib, zeta_s, BQ, beg_wealth, nvec, rpath, wpath, EulDiff)
    bpath = opt.fsolve(LfEulerSys, b_guess, args=(eullf_objs),
                       xtol=TPI_tol)
    cpath, c_cnstr = c10ssf.get_cvec(rpath, wpath, zeta_s, BQ,
    ...
    b_err_vec = c10ssf.get_b_errors(b_err_params, rpath[1:], cpath, bpath,
                                   c_cnstr, EulDiff)
...
def LfEulerSys(bvec, *args):
    ...
    OBJECTS CREATED WITHIN FUNCTION:
    bvec2        = (p + 1,) vector, remaining savings including initial
                   savings
    ...
    --------------------------------------------------------------------
    '''
    beta, sigma, chib, zeta_s, BQ, beg_wealth, nvec, rpath, wpath, EulDiff = args
    bvec2 = np.append(beg_wealth, bvec)
    cvec, c_cnstr = c10ssf.get_cvec(rpath, wpath, zeta_s, BQ, bvec2, nvec)
    b_err_params = (beta, sigma, chib)
    b_err_vec = c10ssf.get_b_errors(b_err_params, rpath[1:], cvec, bvec2,
                                   c_cnstr, EulDiff)
    return b_err_vec
