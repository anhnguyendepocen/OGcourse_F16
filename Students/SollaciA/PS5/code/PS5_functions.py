...
def get_cvec(r, w, bvec, nvec, zeta_s):
    '''
    --------------------------------------------------------------------
    ...
    b_sp1 = bvec[1:]
    BQ = get_BQ(bvec, r)
    cvec = (1 + r) * b_s + w * nvec - b_sp1 + zeta_s*BQ
    if cvec.min() <= 0:
...
def feasible(params, bvec):
    ...
    if not K_cnstr:
        ...
        cvec, c_cnstr = get_cvec(r, w, bvec2, nvec, zeta_s)
...
def get_b_errors(params, r, cvec, c_cnstr, b_sp1, diff):
    '''
    --------------------------------------------------------------------
    ...
    mu_sp1 = b_sp1 ** (-sigma)
    if diff:
        b_errors_sm1 = (beta * (1 + r) * mu_cp1) - mu_c
        b_errors_S = cvec[-1] ** (-sigma) - chi_b*mu_sp1
        b_errors_sm1[c_cnstr[:-1]] = 9999.
        b_errors_sm1[c_cnstr[1:]] = 9999.
        if c_cnstr[-1]:
            b_errors_S = 9999.
    else:
    ...
    b_errors = np.append(b_errors_sm1, b_errors_S)    
...
def EulerSys(bvec, *args):
    ...
    --------------------------------------------------------------------
    '''
    beta, sigma, nvec, L, A, alpha, delta, chi_b, zeta_s, EulDiff = args
    K, K_cnstr = get_K(bvec)
    if K_cnstr:
        ...
        b_err_vec = get_b_errors(b_err_params, r, cvec, c_cnstr, b_sp1,
                                 EulDiff)
...
def get_SS(params, bvec_guess, graphs = True):
    ...
    beta, sigma, nvec, L, A, alpha, delta, chi_b, zeta_s, SS_tol, EulDiff = params
    ...
    if K1_cnstr is True or c1_cnstr.max() is True:
        ...
        eul_args = (beta, sigma, nvec, L, A, alpha, delta, chi_b, zeta_s, EulDiff)
    ...
    c_ss, c_cnstr = get_cvec(r_ss, w_ss, b_ss2, nvec, zeta_s)
    Y_params = (A, alpha)
    ...
    EulErr_ss = get_b_errors(
        b_err_params, r_ss, c_ss, c_cnstr, b_sp1_ss, EulDiff)
    ...
    if graphs:
        ...
        fig, ax = plt.subplots()
        plt.plot(age_pers[:-1], c_ss, marker='D', label='Consumption')
        plt.plot(age_pers, np.append([0],b_ss), marker='D',
                 label='Savings')
...
def get_cbepath(params, rpath, wpath):
    ...
    cpath       = (S, T+S-2) matrix, time path of the distribution of
                  consumption
    bpath       = (S, T+S-2) matrix, time path of the distribution of
                  savings
    EulErrPath  = (S, T+S-2) matrix, time path of Euler errors
    ...
    DiagMaskb   = (p, p) Boolean identity matrix
    ...
    --------------------------------------------------------------------
    '''
    S, T, beta, sigma, chi_b, zeta_s, nvec, bvec1, b_ss, TPI_tol, EulDiff = params
    cpath = np.zeros((S, T + S - 2))
    bpath = np.append(bvec1.reshape((S, 1)),
                      np.zeros((S, T + S - 3)), axis=1)
    ...
    BQ = (1 + rpath[0])*bvec1[-1]
    cpath[S - 1, 0] = ((1 + rpath[0]) * bvec1[S - 2] +
                       wpath[0] * nvec[S - 1]) + zeta_s[S-1]*BQ - bvec1[S-1]
    for p in range(2, S):
        pl_params = (S, beta, sigma, chi_b, zeta_s[-p:], TPI_tol, EulDiff)
...
def paths_life(params, beg_age, beg_wealth, nvec, rpath, wpath,
    ...
    --------------------------------------------------------------------
    '''
    S, beta, sigma, chi_b, zeta_s, TPI_tol, EulDiff = params
    p = int(S - beg_age + 1)
...
def LfEulerSys(bvec, *args):
    ...
    b_err_params = length 2 tuple, (beta, sigma)
    b_err_vec    = (p,) vector, Euler errors from lifetime
                   consumption vector
    ...
    --------------------------------------------------------------------
    '''
    beta, sigma, chi_b, zeta_s, beg_wealth, nvec, rpath, wpath, EulDiff = args
    bvec2 = np.append(beg_wealth, bvec)
    cvec, c_cnstr = get_cvec(rpath, wpath, bvec2, nvec, zeta_s)
    b_err_params = (beta, sigma, chi_b)
    b_sp1 = bvec2[-1]
...
def get_TPI(params, bvec1, graphs):
    ...
    r_params = (A, alpha, delta)
    w_params = (A, alpha)
    cbe_params = (S, T, beta, sigma, chi_b, zeta_s, nvec, bvec1, b_ss, TPI_tol,
                  EulDiff)
