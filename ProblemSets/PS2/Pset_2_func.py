...
def get_cvec(bvec, w, r):
    ...
    cvec = np.array([1, 1, 0.2]) * w + (np.ones(3) + r) * np.concatenate(([0], bvec))\
            - np.append(bvec, [0])
    return cvec
...
def feasible(f_params, bvec_guess):
    ...
    consumption = get_cvec(bvec_guess, w, r)
    
    #create boolean for consumption and capital
    c_b = [consumption[0] <= 0, consumption[1] <= 0, consumption[2] <= 0]
    K_b = (K <= 0)
    ...
    if (c_b[0] == True):
        #then b1 is true
        b_b[0] = True
    #if the third consumption is true
    if (c_b[2] == True):
        #then b2 is true
        b_b[1] = True
    #if the second consumption is False
    if (c_b[1] == True):
        #all b_b is False
        b_b = [True, True]
